...
- **`interactionPossible()` is required for registerInteraction buttons** (2026-04-07): Without this function, the engine silently hides the interaction button. Every vanilla script that uses `registerInteraction` defines `interactionPossible(playerIndex, option)` returning `true` or `false, "reason"`. Always include it.
- **You cannot interact with your own ship** (2026-04-07): `Entity():addScript(...)` in `/run` attaches to the ship you're piloting. But pressing F only interacts with OTHER entities. To test entity scripts, attach them to a station: `/run Player().craft.selectedObject:addScript("entity/foo.lua")` (while targeting a station).
...
- **Namespace scripts: UI callbacks must use bare function names** (2026-04-07): When a script has `-- namespace Foo`, Avorion auto-prepends `Foo.` to all callback lookups. So `tab.onShowFunction = "onShowTab"` becomes `Foo.onShowTab` at runtime. If you write `"Foo.onShowTab"`, it becomes `Foo.Foo.onShowTab` — double-namespaced, nil, silent failure. Always use bare names in callback strings for namespaced scripts.
- **Lib module overrides don't work** (2026-04-07): Placing a `tradingmanager.lua` in our mod's `data/scripts/lib/` does NOT properly override the vanilla version. The mod system doesn't overlay lib modules the same way as entity scripts. Even an unmodified copy of the vanilla file breaks `CreateNamespace`. Only entity/sector/player script overrides work.
- **Vanilla file overrides MUST NOT include() EDE modules at file scope** (2026-04-07): When overriding `tradingmanager.lua` or `resourcetrader.lua`, any `include()` of our modules (even lazy-loaded) can break the vanilla `CreateNamespace` pattern. Factories, trading posts, and consumers all fail with "attempt to call method 'CreateNamespace' (a nil value)". Solution: make trade hooks completely self-contained — inline JSON decode, read `Server():getValue()` directly, do math inline. No EDE module imports in vanilla overrides.
- **Always use `addScriptOnce`, never `addScript`** (2026-04-07): `addScript` adds a duplicate copy every time it's called. Duplicates persist in the save file. Use `addScriptOnce` which is a no-op if the script is already attached. To clean up duplicates: `while entity:hasScript("path") do entity:removeScript("path") end`.
- **`Entity()` returns nil in player scripts** (2026-04-07): `Entity()` returns the entity the script is attached to. Player scripts are attached to the Player object, not an entity — so `Entity()` is nil. To find a station the player is trading with, use `Player().craft.selectedObject` or `Player().craft.dockedTo`. This caused silent early returns in our trade hook callbacks.
- **Don't use `callable()` on callback handlers** (2026-04-07): `callable()` marks functions as remotely invocable via `invokeServerFunction`. Callback handlers registered via `registerCallback` are NOT RPCs — marking them callable is unnecessary and a security risk.
- **Callbacks require explicit `registerCallback()`** (2026-04-07): Defining a function with the callback name is NOT enough. You must call `object:registerCallback("callbackName", "functionName")` in `initialize()`. Without this, `sendCallback()` never reaches your script. Pattern: `Player():registerCallback("onTradingManagerBuyFromPlayer", "onTradingManagerBuyFromPlayer")`.
